...
export default function MemberContentGate({ identifier, locale, t, mediaT, caseSummaryT }: { identifier: string; locale: string; t: Dictionary["memberContent"]; mediaT: Dictionary["media"]; caseSummaryT: Dictionary["caseSummary"] }) {
  ...
  return <div className="member-content"><div className="content-heading"><div><span className="content-kicker">{t.membersOnly} · {kindLabel}</span><h1 {...authoredTitleProps(content.title)}>{content.title}</h1><p>{content.summary}</p></div></div><ContentPlayer content={content} t={mediaT}/>{content.media?.filter((item) => item.kind === "image").map((item) => <figure className="member-content-image" key={item.id}><img src={item.publicUrl} alt={item.altText ?? ""}/>{item.caption && <figcaption>{item.caption}</figcaption>}</figure>)}{documents.length ? <section className="content-downloads"><div className="section-mini-head"><div><h2>{t.downloads}</h2></div></div><ul>{documents.map((item) => <li key={item.id}><a href={item.publicUrl} target="_blank" rel="noreferrer">{item.caption || item.altText || t.downloadDocument}</a></li>)}</ul></section> : null}<section className="case-summary-panel"><div className="section-mini-head"><div><h2>{t.caseSummary}</h2></div></div><dl className="case-summary-list">{resolveCaseSections(content, caseSummaryT).map(({ key, label, body }) => <div key={key}><dt>{label}</dt><dd dangerouslySetInnerHTML={{ __html: body }} /></div>)}</dl></section></div>;
}
