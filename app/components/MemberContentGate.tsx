...
export default function MemberContentGate({ identifier, locale }: { identifier: string; locale: string }) {
  ...
  return <div className="member-content"><div className="content-heading"><div><span className="content-kicker">Members-only · {content.kind.replace(/_/g, " ")}</span><h1>{content.title}</h1><p>{content.summary}</p></div></div><ContentPlayer content={content}/>{content.media?.filter((item) => item.kind === "image").map((item) => <figure className="member-content-image" key={item.id}><img src={item.publicUrl} alt={item.altText ?? ""}/>{item.caption && <figcaption>{item.caption}</figcaption>}</figure>)}{documents.length ? <section className="content-downloads"><div className="section-mini-head"><div><span className="section-kicker">Resources</span><h2>Downloads</h2></div></div><ul>{documents.map((item) => <li key={item.id}><a href={item.publicUrl} target="_blank" rel="noreferrer">{item.caption || item.altText || "Download document"}</a></li>)}</ul></section> : null}<section className="case-summary-panel"><div className="section-mini-head"><div><span className="section-kicker">Clinical record</span><h2>Case summary</h2></div></div><dl className="case-summary-list">{CASE_SUMMARY_FIELDS.map(({ key, label }) => content.caseSummary?.[key] ? <div key={key}><dt>{label}</dt><dd dangerouslySetInnerHTML={{ __html: content.caseSummary[key] }} /></div> : null)}</dl></section></div>;
}
