...
import { eventDateRange, eventFormatLabel, eventTypeLabel, type TeamEvent } from "../lib/event-data";
...
export default function EventsExplorer({ locale, events, t }: { locale: Locale; events: TeamEvent[]; t: Dictionary["events"] }) {
  ...
  const typeLabel = (value: string) => eventTypeLabel(value, t);
  ...
  const formatLabel = (value: string) => eventFormatLabel(value, t);
  ...
  return <section className="events-collection" aria-labelledby="all-events-heading">
    ...
    <div aria-live="polite">
      {upcoming.length > 0 && <section className="event-group" aria-labelledby="upcoming-events-heading"><div className="event-group-heading"><span className="event-group-marker event-group-marker-upcoming"/><div><h3 id="upcoming-events-heading">{t.upcomingEvents}</h3></div></div><div className="event-list">{upcoming.map(renderEvent)}</div></section>}
      {past.length > 0 && <section className="event-group event-group-past" aria-labelledby="past-events-heading"><div className="event-group-heading"><span className="event-group-marker event-group-marker-past"/><div><h3 id="past-events-heading">{t.pastEvents}</h3></div></div><div className="event-list">{past.map(renderEvent)}</div></section>}
