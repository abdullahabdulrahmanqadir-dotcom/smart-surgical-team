...
export default function MemberProfile({ locale, initialMember, t }: { locale: string; initialMember: Member; t: Dictionary["profile"] }) {
  ...
  const [fullName, setFullName] = useState(initialMember?.name ?? "");
      ...
      <div className="profile-content">
        ...
        <section className="profile-panel profile-details" aria-labelledby="profile-details-title">
          ...
          <form className="profile-details-form" onSubmit={saveProfile} noValidate>
            <div className="form-field"><label htmlFor="profile-full-name">{t.fullName}</label><div className="field-control"><IconUser size={18} /><input id="profile-full-name" name="fullName" autoComplete="name" value={fullName} onChange={(event) => { setFullName(event.target.value); setProfileStatus(null); }} required /></div></div>
            <div className="form-field"><label htmlFor="profile-email">{t.emailAddress}</label><div className="field-control is-readonly"><IconMail size={18} /><input id="profile-email" type="email" value={member.email} readOnly aria-describedby="profile-email-help" /></div><small id="profile-email-help">{t.emailReadOnly}</small></div>
