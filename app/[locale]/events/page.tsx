...
import { eventDateRange, eventDateStamp, getPublicEvents } from "../../lib/events";
...
export default async function EventsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <><a className="skip-link" href="#main-content">{dict.nav.skipToContent}</a><SiteHeader locale={active} dict={dict}/><main id="main-content">
    ...
    {featured && <section className="featured-event-section"><div className="featured-event"><div className="featured-event-art">{featured.image && <img src={featured.image} alt=""/>}<div className="featured-event-stamp"><span>Featured event</span><strong>{eventDateStamp(featured, active).days}<br/>{eventDateStamp(featured, active).month}</strong></div></div><div className="featured-event-copy"><div className="event-card-tags"><span className={`event-status event-status-${featured.status}`}>{featured.status === "past" ? "Past event" : "Upcoming"}</span><span>{featured.type}</span></div><h2>{featured.title}</h2><p>{featured.summary}</p><div className="featured-event-meta"><span><IconCalendar size={18}/>{eventDateRange(featured, active)}</span><span><IconPin size={18}/>{featured.location}</span></div><Link className="btn btn-primary" href={localePath(active, `events/${featured.slug}`)}>Explore the summit <IconArrowRight size={17}/></Link></div></div></section>}
