...
export default async function EventsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <><a className="skip-link" href="#main-content">{dict.nav.skipToContent}</a><SiteHeader locale={active} dict={dict}/><main id="main-content">
    <section className="events-hero"><div className="events-hero-photo" aria-hidden="true"><img src="/events/smart-health-tower-events-hero.webp" alt=""/></div><div className="events-hero-inner"><div className="events-hero-copy"><span className="eyebrow">{dict.events.heroEyebrow}</span><h1>{dict.events.heroTitle}<br/><span>{dict.events.heroAccent}</span></h1><p>{dict.events.heroIntro}</p><a className="btn btn-primary btn-lg" href="#all-events">{dict.events.browseEvents} <IconArrowRight size={18}/></a></div></div></section>
    {featured && <section className="featured-event-section"><div className="featured-event"><div className="featured-event-art">{featured.image && <img src={featured.image} alt=""/>}<div className="featured-event-stamp"><span>{dict.events.featuredEvent}</span><strong>{eventDateStamp(featured, active).days}<br/>{eventDateStamp(featured, active).month}</strong></div></div><div className="featured-event-copy"><div className="event-card-tags"><span className={`event-status event-status-${featured.status}`}>{featured.status === "past" ? dict.events.pastEvent : dict.events.upcoming}</span><span>{featured.type}</span></div><h2 {...authoredTitleProps(featured.title)}>{featured.title}</h2><p>{featured.summary}</p><div className="featured-event-meta"><span><IconCalendar size={18}/>{eventDateRange(featured, active)}</span><span><IconPin size={18}/>{featured.location}</span></div><Link className="btn btn-primary" href={localePath(active, `events/${featured.slug}`)}>{dict.events.exploreSummit} <IconArrowRight size={17}/></Link></div></div></section>}
