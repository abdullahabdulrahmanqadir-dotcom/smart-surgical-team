...
export default async function EventsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const hero = featured
    ? { title: dict.events.heroTitle, accent: dict.events.heroAccent, intro: dict.events.heroIntro, cta: dict.events.browseEvents }
    : { title: dict.events.archiveHeroTitle, accent: dict.events.archiveHeroAccent, intro: dict.events.archiveHeroIntro, cta: dict.events.browseArchive };
  return <><a className="skip-link" href="#main-content">{dict.nav.skipToContent}</a><SiteHeader locale={active} dict={dict}/><main id="main-content">
    <section className="events-hero"><div className="events-hero-photo" aria-hidden="true"><img src="/events/smart-health-tower-events-hero.webp" alt=""/></div><div className="events-hero-inner"><div className="events-hero-copy"><h1>{hero.title}<br/><span>{hero.accent}</span></h1><p>{hero.intro}</p><a className="btn btn-primary btn-lg" href="#all-events">{hero.cta} <IconArrowRight size={18}/></a></div></div></section>
