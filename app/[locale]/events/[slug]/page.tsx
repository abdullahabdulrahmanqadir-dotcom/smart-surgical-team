...
import { eventTypeLabel, eventDateRange, getPublicEvent, localizeFallbackEvent } from "../../../lib/events";
...
export default async function EventDetailPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const event = localizeFallbackEvent(sourceEvent, dict.eventFallback);
  // Was a ternary chain that fell through to "Conference" for any unknown
  // type; the shared helper keeps the original word instead.
  const eventType = eventTypeLabel(event.type, dict.events);
